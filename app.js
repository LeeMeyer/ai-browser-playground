...
const CHROME_PLAN_REQUEST_OPTIONS = {
  responseConstraint: PLAN_SCHEMA,
  omitResponseConstraintInput: true,
};

const CHROME_SYSTEM_PLAN_INSTRUCTION =
...
async function initChromeAi() {
  ...
  state.chromeSession = await LanguageModel.create({
    ...
    initialPrompts: [
      {
        role: "system",
        content: CHROME_SYSTEM_PLAN_INSTRUCTION,
      },
    ],
  });
...
async function initTransformersAi() {
  ...
  if (!navigator.gpu) {
    setStatus("WebGPU unavailable. Loading Transformers.js with CPU/WASM fallback...", "warning");
  }

  const sourceLabel = state.cacheSourceHint;
  setStatus(`Loading Transformers.js model: ${preset.modelId}${sourceLabel ? ` (${sourceLabel})` : ""}`, "warning");
  ...
  try {
    ...
    const timedOut = /timed out after\s*\d+s/i.test(message);

    if (timedOut) {
      setStatus("Download is taking longer than expected. Retrying once...", "warning");
      try {
        const [processor, model] = await withTimeout(
          Promise.all([
            AutoProcessor.from_pretrained(preset.modelId, {
          ...
          MODEL_INIT_TIMEOUT_MS,
          "Transformers model initialization retry"
        );
        ...
        syncReadyUi();
        syncInitButtonUi();
        throw new Error(`Unable to load ${preset.label} in Transformers.js after retry: ${String(retryError?.message || retryError || "unknown error")}`);
      }
    }
...
function buildPlannerPrompt(input, context) {
...
function buildChromePlannerPrompt(input, current) {
  return buildPlannerPrompt(input, current);
}

function buildTransformersPlannerPrompt(input, current) {
  const context = getModelCodeContext(current);
  return buildPlannerPrompt(input, context);
}

function parsePlannerResponse(rawResponseText) {
  return normalizePlan(extractJson(rawResponseText));
}

async function promptChromeForPlan(promptText) {
  return state.chromeSession.prompt(promptText, CHROME_PLAN_REQUEST_OPTIONS);
}
...
async function getPlanFromChrome(input, current) {
  const plannerPrompt = buildChromePlannerPrompt(input, current);
  const raw = await promptChromeForPlan(plannerPrompt);
  return { raw, plan: parsePlannerResponse(raw) };
}

async function getPlanFromTransformers(input, current) {
  const plannerPrompt = buildTransformersPlannerPrompt(input, current);
  const raw = await promptTransformers(plannerPrompt);
  return { raw, plan: parsePlannerResponse(raw) };
}
...
async function handleUserPrompt(input) {
  ...
  const current = await getCurrentCode();
  const firstAttempt =
    state.provider === "chrome" ? await getPlanFromChrome(input, current) : await getPlanFromTransformers(input, current);
